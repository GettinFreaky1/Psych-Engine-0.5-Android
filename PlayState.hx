...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		readyScreen = new FlxSprite(282.5, 211).loadGraphic(Paths.image('ready'));
		readyScreen.scrollFactor.set();
		readyScreen.updateHitbox();
		add(readyScreen);
		readyScreen.scale.set(1, 1); // Lets gooo fucking poggers
		FlxTween.tween(readyScreen.scale, {x: 0.9, y:0.9}, 1, {type:FlxTween.PINGPONG, startDelay: 0.5});
	...
	var readyScreen:FlxSprite;
	var notStartYet:Bool = true;
	...
	function startingCountdown()
	{
		...
		trace("Fucking lets goo poggers");
	}
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (i.overlaps(readyScreen) && i.justPressed)
			{
				startingCountdown();
			}
		}
		...
		if (FlxG.keys.justPressed.ANY && notStartYet)
		{
			startingCountdown();
		}
		...
		if (controls.PAUSE #if android || FlxG.android.justReleased.BACK #end && youCanPauseNow && canPause)
		{
